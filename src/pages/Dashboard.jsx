...
function DashboardContent({ blogger: bloggerProp, onLogout }) {
  ...
  const D = {
    ...
    commStructure:[{val:'USD',struct:'Revenue Share',group:'RS25% REF10%',start:'2026-06-02',desc:'Procent comision: 25%; Comision negativ: Da; Administrator: 0%',end:''}],
  }
  ...
  // Bloggeri invitați (referral 10%) — încărcați din Firebase la 10s
  useEffect(() => {
          ...
          {page==='comm'&&(
            <div>
              ...
              <div style={{background:'linear-gradient(135deg,rgba(245,166,35,0.1),rgba(245,166,35,0.03))',border:'1px solid rgba(245,166,35,0.3)',borderRadius:10,padding:'16px 20px',marginBottom:'1.25rem'}}>
                <div style={{fontSize:14,fontWeight:700,color:gold,marginBottom:8}}>📋 {L({ro:'Structura ta de comision: RS25% + REF10%',ru:'Твоя структура комиссий: RS25% + REF10%',en:'Your commission structure: RS25% + REF10%',tr:'Komisyon yapın: RS25% + REF10%',de:'Deine Provisionsstruktur: RS25% + REF10%',pt:'A tua estrutura de comissões: RS25% + REF10%',pl:'Twoja struktura prowizji: RS25% + REF10%'})}</div>
                <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:12}}>
                  ...
                  <div style={{fontSize:12,color:txtSub,lineHeight:1.7}}>
                    <b style={{color:'#10b981'}}>Referral 10%</b> {L({ro:'— dacă inviți un alt blogger în WinPartners, primești 10% din comisionul lui lunar, pe viață. Copiază linkul de referral din tab-ul Link-uri Afiliați.',ru:'— если приглашаешь другого блогера в WinPartners, получаешь 10% с его месячной комиссии, пожизненно. Скопируй реферальную ссылку во вкладке Партнёрские ссылки.',en:'— if you invite another blogger to WinPartners, you get 10% of their monthly commission, for life. Copy your referral link from the Affiliate Links tab.',tr:'— WinPartners e başka bir blogcu davet edersen, onun aylık komisyonunun %3 ünü ömür boyu alırsın. Referans bağlantını Ortaklık Bağlantıları sekmesinden kopyala.',de:'— wenn du einen anderen Blogger zu WinPartners einlädst, erhältst du 10% seiner monatlichen Provision, lebenslang. Kopiere deinen Empfehlungslink im Tab Affiliate-Links.',pt:'— se convidares outro blogger para a WinPartners, recebes 10% da comissão mensal dele, para sempre. Copia o teu link de indicação no separador Links de Afiliado.',pl:'— jeśli zaprosisz innego blogera do WinPartners, otrzymasz 10% jego miesięcznej prowizji, dożywotnio. Skopiuj link polecający w zakładce Linki Afiliacyjne.'})}
                  </div>
                </div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:12,marginBottom:'1.5rem'}}>
                <div style={{background:'linear-gradient(135deg,rgba(16,185,129,0.12),rgba(16,185,129,0.04))',border:'1px solid rgba(16,185,129,0.3)',borderRadius:10,padding:'14px 18px',display:'flex',gap:12,alignItems:'flex-start'}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:txtSub,lineHeight:1.6}}>{L({ro:'Primești 25% din pierderile jucătorilor tăi + 10% bonus din câștigurile oricărui blogger pe care îl inviți tu în program — pe viață.',ru:'Получаешь 25% с проигрышей твоих игроков + 10% бонус с доходов любого блогера, которого ты пригласишь в программу — пожизненно.',en:'You get 25% of your players losses + a 10% bonus from the earnings of any blogger you invite to the program — for life.',tr:'Oyuncularının kayıplarının %25 i + programa davet ettiğin her blogcunun kazancından %3 bonus — ömür boyu.',de:'Du erhältst 25% der Verluste deiner Spieler + 10% Bonus aus den Einnahmen jedes Bloggers, den du ins Programm einlädst — lebenslang.',pt:'Recebes 25% das perdas dos teus jogadores + 10% de bónus dos ganhos de qualquer blogger que convidares para o programa — para sempre.',pl:'Otrzymujesz 25% strat swoich graczy + 10% bonusu z zarobków każdego blogera, którego zaprosisz do programu — dożywotnio.'})}</div>
                  </div>
                </div>
          ...
          {page==='subaff'&&(
            <div>
              <div style={{background:'#10231a',border:'1px solid #bbf7d0',borderRadius:8,padding:'14px 16px',marginBottom:'1.25rem'}}>
                <div style={{fontSize:13,fontWeight:600,color:'#065f46',marginBottom:5}}>💰 {L({ro:'Câștigă 10% din comisioanele bloggerilor pe care îi inviți — pe viață!',ru:'Зарабатывай 10% с комиссий приглашённых блогеров — пожизненно!',en:'Earn 10% of the commissions of bloggers you invite — for life!',tr:'Davet ettiğin blogcuların komisyonlarının %3 kazan — ömür boyu!',de:'Verdiene 10% der Provisionen der von dir eingeladenen Blogger — lebenslang!',pt:'Ganha 10% das comissões dos bloggers que convidares — para sempre!',pl:'Zarabiaj 10% z prowizji zaproszonych blogerów — dożywotnio!'})}</div>
